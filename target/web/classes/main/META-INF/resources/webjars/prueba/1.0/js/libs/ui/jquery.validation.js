...
!function($) {
	...
	function Validation($this, req) {
		...
		this.req = req || function() {
			if(this.$this.hasClass("noValidar")) { 
				return true;
			} 

			else if(this.$this.is('[type=checkbox] , [type=radio]')) {
			...
			else if(this.$this.is('input')) {
			   return !!this.$this.val().length;
			} 

			else if(this.$this.is('select')) {
               if((this.$this.val()==null)||(this.$this.size()==0) || (this.$this.val()=='0')){
                   return false;
               } else {
               	   return true;
               }
